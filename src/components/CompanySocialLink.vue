<template>
  <FormKit type="form" @submit="(data) => $emit('save', data)" form-class="h-full overflow-y-scroll" :actions=false :incomplete-message=false :value="company">
        ...
        <div class="mt-6 grid grid-cols-1 gap-y-6">
          <div class="flex flex-col lg:flex-row gap-6 lg:w-full lg:gap-28 lg:content-start">
            ...
            <div class="lg:basis-96 flex flex-col gap-6 grow max-w-xl">
              <FormKit v-for="s in socials" :name="s" :id="s" type="url" :label="s" inner-class="mt-1" label-class="capitalize" />
            </div>
          </div>

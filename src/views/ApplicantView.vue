<template>
  <div class="flex h-full">
    <div class="border-r border-soft max-w-sidebar w-full p-2 h-full" v-if="candidate">
      ...
      <div class="my-4">
        <RouterLink v-for="item in applicantSidebar" :to="{name: item.name}" :class="[$route.name == item.name ? 'bg-soft' :'','h-8 flex items-center px-2 hover:bg-mute block rounded-lg text-color-text text-sm']">
          <i class="mr-2">
